import ts from "https://esm.sh/typescript@5.4.2";

function isNamespace(node: ts.Node | undefined): node is ts.ModuleDeclaration {
    return !!(node && ts.isModuleDeclaration(node) && node.body && ts.isModuleBlock(node.body));
}

function getNamespaceName(node: ts.ModuleDeclaration): string {
