...
import { knownLibFiles } from "./known-lib-files.ts";
...
export class InMemorySystem implements AnalyserSystem {
  ...
  getDirectories(path: string): string[] {
    ...
    return node?.type === FSNodeType.Directory
      ? node.children
          .filter((d) => d.type === FSNodeType.Directory)
          .map((d) => d.name)
      : [];
  }
  ...
  readDirectory(path: string, extensions?: readonly string[]): string[] {
    ...
    const fileNames = node.children
      .filter((f) => f.type === FSNodeType.File)
      .map((f) => this.join(basePath, f.name));
  ...
  deleteFile(path: string): void {
    ...
    if (node.type === FSNodeType.Directory) {
      throw new Error(
        `Unable to delete the file "${absolutePath}" because it's a directory.`
      );
    }
  ...
  private async _writeLibFilesFromNodeModules(): Promise<void> {
    const m = await import("./get-lib-files-from-node.ts");
  ...
  private _setFile(absolutePath: string, data: string): void {
    ...
    if (!newFileName) {
      throw new Error(
        `Empty file name found when trying to write to: "${absolutePath}".`
      );
    }
    ...
    for (const directoryName of parentDirectories) {
      ...
      if (child?.type === FSNodeType.File) {
        throw new Error(
          `Expected a directory but found a file when trying to write to: "${absolutePath}".`
        );
      }
