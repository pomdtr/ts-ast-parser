...
import type { FunctionLikeNode, NodeWithFunctionDeclaration, SymbolWithContext } from "../utils/types.ts";
...
import type { Method, ModifierType } from "../models/member.ts";
...
export class FunctionNode implements DeclarationNode<FunctionDeclaration | Method, NodeWithFunctionDeclaration> {
    ...
    private _getFunctionNode(): FunctionLikeNode | null {
        ...
        if (ts.isPropertySignature(this._node)) {
            func = this._node.type && ts.isFunctionTypeNode(this._node.type) ? this._node.type : null;
        }
